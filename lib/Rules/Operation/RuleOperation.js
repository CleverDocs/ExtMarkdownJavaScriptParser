...
class RuleOperation extends require("../../Rule") {
  ...
  constructor() {
    super();
    this.operationPattern = /(\{{2}|\({2})(.*?[\s\S]*?)(\}{2}|\){2})/g;
    this.refPattern = /[A-Za-z-_0-9]{1,}\=/g;
    this.chainUndefined = "??";
  }
   ...
   */
  parse(eMdDocument) {
    let newEMdElementList = this.matchAndSplit(eMdDocument, this.operationPattern, function(text, elementNumber) {
      let eMdElement = new EMdElementOperation(text, elementNumber, eMdDocument.getFullOriginalText());
      return eMdElement;
    });
  ...
  execute(eMdElementList) {
    ...
    //Debug purposes
    /*eMdElementList.map(function(el){
      if( el.type == "operation" ){
        console.log(el.getOperationType());
      }
    });*/
    ...
    for (var op of eMdElementList) {
      if (op instanceof EMdElementOperation) {
          ...
          if (eMdElementList[varCollectorCounter]instanceof EMdElementOperation) {
            if (eMdElementList[varCollectorCounter].isExecutionSuccess() || eMdElementList[varCollectorCounter].isExecuted() == false) {
              for (var instruction of eMdElementList[varCollectorCounter].instructions) {
                var varStackInst = instruction.translated.match(this.refPattern);
        ...
        try {
          ...
          op.addException(new RuntimeErrorEMdOperationException(e.message, op.getElementNumber()));
        }
    ...
    eMdElementList.map(function(eMdElement){
      if (eMdElement instanceof EMdElementOperation) {
        eMdElement.setOutput(varToReturnList[i]);
        i++;
      }
    });
    ...
    eMdElementList.map(function(eMdElement){
      if (eMdElement instanceof EMdElementOperation) {
        eMdElement.processAndSetOutput( eMdElementList );
      }
    });
