...
class RuleOperation extends Rule {
  ...
  parseAndGetObjects(md){
    ...
    var output = this.operationRunner.run(opsFound);
    var finalObjects = this.operationMatcher.finalGetObjects(md, output, this, this.tagsEnabled);

    var i = 0;
    for (var ob of finalObjects) {
      if( ob.type == "operation" ){
        //@CONTINUE
        //I don' know why but the returns [Object] instead of JSon
        //   { type: 'operation',
       //original: ' P€ ',
       //content: '',
       //exceptions: [Object],
       //isInline: true },
     //{ type: 'string',
     //Next step after this : The last red Mocha test
        ob.exceptions = opsFound[i].getEMdExceptions(true);
        ob.isInline = opsFound[i].isInLine();
        i++;
      }
    }
