/*!
 * OperationMatcher
 *
 * Object that brings methods to match operations inside a string
 *
 * @param {String} md String which expected to contains eMd
 *
 * Copyright(c) 20017 Jean-Luc Ranaivoarivao <ranaivoarivaojl@me.com>
 * MIT Licensed
 */

const cheerio = require('cheerio')
var Operation = require("./operation");
var ConcatenationManager = require("./../../tools/concatenationManager");
var OperationRunner = require("./operationRunner");

class OperationMatcher {

  constructor(md) {
    this.operationPattern = /\{\{(.*?[\s\S]*?)\}\}/g;
    this.operationRunner = new OperationRunner();
  }

  /**
   * process()
   * @TODO Set docs
   */
  process(md, rule){
    var opsFound = [];

    var opsMatched = md.match(this.operationPattern);

    var i = 0;
    if (opsMatched) {
      for (var ope of opsMatched) {
        opsFound.push(new Operation(ope, i, md));
        i++;
      }
    }

    var output = this.operationRunner.run(opsFound);
    var finalObjects = this.finalGetObjects(md, output, rule, rule.tagsEnabled);

    var i = 0;
    var j = 0;
    for (var ob of finalObjects) {
      if( ob.type == "operation" ){
        opsFound[j].setContent( finalObjects[i].content );
        finalObjects[i] = opsFound[j];
        j++;
      }
      i++;
    }

    return finalObjects;
  }

  /**
   * finalGetObjects()
   *
   * @parameter {String} md Markdown string of the operation
   * @parameter {Array} operationsOutput Array composed with operation output
   * @parameter {Rule}
   * @parameter {Bool} set if concatenationManager must add tags or not
   * @return {String} md New md
   */
  finalGetObjects(md, operationsOutput, rule, tagsEnabled){

    var arrayToReturn = [];
    var splitedMd = md.split(this.operationPattern);
    var i = 1;
    var iOperations = 0;

    for (var mdPart of splitedMd) {
      if (i % 2 == 0) {
        var concatenationManager = new ConcatenationManager(arrayToReturn, operationsOutput[iOperations], splitedMd[i], rule, iOperations+1, tagsEnabled);
        arrayToReturn.push( this.setObject( mdPart, concatenationManager.get(), true ) );

        splitedMd[i] = concatenationManager.getAfterString();
        //arrayToReturn[iOperations] = this.setObject( concatenationManager.getBeforeString(), null, false, null );
        //@CONTINUE OK, there's an issue here ... perhaps you'll have to redo contactenation manager to get it works with array instead of string ?

        iOperations++;
      } else {
        arrayToReturn.push( this.setObject( mdPart, null, false, null ) );
      }
      i++;
    }

    console.log(arrayToReturn);

    return arrayToReturn;
  }

  /**
   * setObject()
   * Method that set the item that will be pushed into the array as result of
   * finalGetObjects()
   *
   * @TODO I think this function must be implemented inside operation object
   *
   * @param {String}  originalString
   * @param {String}  resultString
   * @param {Boolean} isOperation
   */
  setObject( originalString, resultString, isOperation, isInline ){
    var objectToReturn;

    if( isOperation ){
      objectToReturn = {
        type: "operation",
        original: originalString,
        content: resultString,
        exceptions: []
      };
    }else{
      objectToReturn = {
        type: "string",
        content: originalString
      };
    }

    return objectToReturn;
  }
}

module.exports = OperationMatcher;
