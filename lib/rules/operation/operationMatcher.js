...
class OperationMatcher {
   ...
   */
  finalReplace(md, operationsOutput) {
    var result = "";
    ...
    for (var mdPart of splitedMd) {
      if (i % 2 == 0) {
        var whitespaceManager = new WhitespaceManager(result, operationsOutput[iOperations], splitedMd[i]);
        result = whitespaceManager.get();
        splitedMd[i] = whitespaceManager.getAfterString();
        iOperations++;
      } else {
        result += mdPart;
      }
