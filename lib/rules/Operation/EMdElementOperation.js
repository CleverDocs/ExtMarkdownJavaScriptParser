...
const RefOutputOnlyOneInstructionSyntaxChecker = require("./SyntaxChecker/RefOutputOnlyOneInstructionSyntaxChecker");
const RefOutputReferenceOnlySyntaxChecker = require("./SyntaxChecker/RefOutputReferenceOnlySyntaxChecker");
...
class EMdElementOperation extends EMdElement {
  ...
  constructor(originalOperationText, elementNumber, eMdDocument) {
    ...
    this.syntaxCheckers = [ new UnknowCharPreventSyntaxChecker(),
                            new RefOutputOnlyOneInstructionSyntaxChecker(),
                            new RefOutputReferenceOnlySyntaxChecker()];

    this.processedText = originalOperationText;
    this.output = originalOperationText;

    this.setOperationType(); //Never do this before removeSeparators()
  ...
  removeSeparators() {
    //Remove {{ }} and (( ))
    this.processedText = this.processedText.replace(/\{{2}/g, "");
    this.processedText = this.processedText.replace(/\}{2}/g, "");
    this.processedText = this.processedText.replace(/\({2}/g, "");
    this.processedText = this.processedText.replace(/\){2}/g, "");
  }
  ...
  setOperationType() {
    if( this.operationType === undefined ){
      if( this.processedText.split('')[0] == "(" ){
        this.operationType = "ref-output";
      }else if( this.processedText.split('')[0] == "[" ){
        this.operationType = "table";
      }else{
   ...
   * Can be simple, or ref-output or table
   *
   * @return {Boolean}
   */
  getOperationType() {
