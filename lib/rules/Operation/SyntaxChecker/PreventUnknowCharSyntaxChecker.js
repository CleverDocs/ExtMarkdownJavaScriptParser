    ...
    for (var instructionsOpe of ope.getInstructions()) {
      ...
      for (var charOpe of instructionsOpe.characters) {
        if (!charOpe.character.match(this.acceptedCharsPattern)) {
          ope.addException(new SyntaxErrorEMdOperationException("Unknow character '" + charOpe.character + "' on operation " + charOpe.position.operationNumber + ", line " + charOpe.position.line + " and column " + charOpe.position.column, ope.getOperationNumber(), charOpe.position.line, charOpe.position.column ));
        }
      }
    }
  }
