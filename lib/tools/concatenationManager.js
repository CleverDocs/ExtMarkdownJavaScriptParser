...
class ConcatenationManager {
  ...
  constructor(currentArrayConcatenated, replacerString, afterString, rule, operationNumber, tagsEnabled) {
    ...
    this.currentArrayConcatenated = currentArrayConcatenated;
  ...
  removeSimpleWhitespace() {
    ...
    var previousChar = this.currentArrayConcatenated[this.currentArrayConcatenated.length-1].content.slice(-1);
    var nextChar = this.afterString.charAt(0);

    if ((previousChar == " " && nextChar == " ") || (previousChar == " " && (nextChar == undefined || nextChar == "" || nextChar == "\n"))) {
      this.currentArrayConcatenated[this.currentArrayConcatenated.length-1].content = this.currentArrayConcatenated[this.currentArrayConcatenated.length-1].content.substring(0, this.currentArrayConcatenated[this.currentArrayConcatenated.length-1].content.length - 1);
  ...
  removeMarkdownWhitespace() {
    ...
    var tempString = this.currentArrayConcatenated[this.currentArrayConcatenated.length-1].content + randomString + this.afterString;
